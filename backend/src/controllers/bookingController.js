  ...
  try {
    ...
    const [conflicts] = await pool.query(
      `SELECT id FROM bookings
       WHERE motorcycle_id = ? AND status IN ('pending', 'approved', 'cancel_requested')
    ...
    await notifyAdmins(
      'New Booking Request',
      `${booking.customer_name} requested to book ${motorcycle.brand} ${motorcycle.model}.`,
      'new_booking',
      result.insertId
    );
...
exports.cancel = async (req, res, next) => {
  try {
    ...
    if (bookings[0].status !== 'pending') {
      return res.status(400).json({ success: false, message: 'Only pending bookings can be directly cancelled.' });
    }
...
exports.requestCancel = async (req, res, next) => {
  try {
    ...
    if (bookings[0].status !== 'approved') {
      return res.status(400).json({ success: false, message: 'Only approved bookings can request cancellation.' });
    }

    const booking = bookings[0];
    await pool.query("UPDATE bookings SET status = 'cancel_requested' WHERE id = ?", [req.params.id]);

    await notifyAdmins(
      'Cancel Request',
      `${booking.customer_name} has requested to cancel their booking for ${booking.brand} ${booking.model}.`,
      'cancel_requested',
      booking.id
    );

    res.json({ success: true, message: 'Cancellation request submitted. Awaiting admin review.' });
  } catch (err) {
    next(err);
  }
};
...
exports.updateStatus = async (req, res, next) => {
  try {
    ...
    const [bookings] = await pool.query(
      `SELECT b.*, m.brand, m.model
       FROM bookings b JOIN motorcycles m ON b.motorcycle_id = m.id
       WHERE b.id = ?`,
      [req.params.id]
    );
    if (bookings.length === 0) {
    ...
    if (status === 'approved' && previousStatus === 'pending') {
      await createNotification(
        booking.user_id,
        'Booking Approved!',
        `Your booking for ${bike} has been approved. Enjoy your ride!`,
        'booking_approved',
        booking.id
      );
      ...
      await createNotification(
        booking.user_id,
        'Booking Rejected',
        `Your booking for ${bike} was not approved. Please contact us for more details.`,
        'booking_rejected',
        booking.id
      );
    } else if (status === 'cancelled' && previousStatus === 'cancel_requested') {
      await createNotification(
        booking.user_id,
        'Cancellation Approved',
        `Your cancellation request for ${bike} has been approved.`,
        'cancel_approved',
        booking.id
      );
    } else if (status === 'approved' && previousStatus === 'cancel_requested') {
      await createNotification(
        booking.user_id,
        'Cancel Request Declined',
        `Your cancellation request for ${bike} was declined. Your booking remains active.`,
        'cancel_rejected',
        booking.id
      );
    }
